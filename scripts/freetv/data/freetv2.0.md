...
1. 配置硬编码：数据写在代码里，修改需要动代码
2. 不够灵活：每次改源或频道都要改Python文件
3. 维护困难：频道列表更新需要重新编辑脚本
...
├── config.json (配置文件)
└── data/
    ├── rename_rules.txt (重命名规则)
    ├── channel_list.txt (总频道列表)
    ├── cctv_list.txt (CCTV列表)
    └── ws_list.txt (卫视列表)
```
...
· 开箱即用：第一次运行自动创建所有文件
· 灵活配置：用户可以轻松修改外部文件，无需改代码
· 易于更新：更新频道列表只需编辑txt文件
· 版本安全：核心逻辑不受用户配置影响
